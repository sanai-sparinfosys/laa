...
export class ClientPageComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    this._httpService.getImages().subscribe((res) => {
      ...
      this._httpService.getValidData(this.images[0].aws_image_path).subscribe((data) => {
        this.loader = false;
  ...
  paginationChange(event){
    ...
    this._httpService.getValidData(image).subscribe((data) => {
      if(Object.keys(data).length != 0){
        this.allData = data.response;
        this.machedData = this.allData.filter((el) => {
          return el.Status.indexOf("Matched With") >= 0;
  ...
  applyFilter(filterValue) {
    this._httpService.getValidData(filterValue.source.value).subscribe((data) => {
      if(Object.keys(data).length != 0){
        this.allData = data.response;
        this.machedData = this.allData.filter((el) => {
          return el.Status.indexOf("Matched With") >= 0;
